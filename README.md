...
6、如何支撑上亿链接以及上万亿qps的高并发实时系统架构
   1、首先根据DDD领域驱动模型来进行系统的设计，分为显示层，接入层，逻辑层，基础服务层，假设每台机器配置为8c16g，每台机器支撑10w链接
   2、首先接入流量的时候进行对接入层网关进行链接分配均分到1000台服务器
   3、经过测试每台机器的每个连接qps可以达到每秒5w，这里接入qps最高能达到5万亿
   4、然后网关和内部服务之间已经建立好长连接池，不需要担心三次握手四次挥手的性能消耗
   5、在逻辑层以及基础服务层可以根据相应的操作来消耗掉这5万亿qps，最终的压力就在于基础服务层，可以根据不同的业务对基础服务层进行水平或者垂直扩容
